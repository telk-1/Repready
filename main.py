...
RULES BY GOAL/LEVEL (apply strictly):
...
Goal: {p.get('goal','General Fitness')} | Level: {p.get('level','Intermediate')} | Days: {p.get('days','4')}/week | Duration: {p.get('duration','60')}min
...
{progression_context}
CRITICAL: Respond ONLY with valid complete JSON. No markdown, no backticks, no trailing commas:
{{"planName":"string","coachMessage":"1 sentence to {p.get('name','you')} referencing their progress and why this plan is their next step","overview":"2 sentences on what and why","weeklyVolume":0,"estimatedCalories":0,"days":[{{"day":"Day 1 - Focus","focus":"string","warmup":"string","exercises":[{{"name":"string","sets":3,"reps":"8-10","weight":"70kg","rest":"90s","notes":"why this exercise"}}],"cooldown":"string"}}],"nutritionPlan":{{"dailyCalories":0,"protein":0,"carbs":0,"fats":0,"meals":[{{"meal":"Breakfast","foods":"brief","calories":0}},{{"meal":"Lunch","foods":"brief","calories":0}},{{"meal":"Dinner","foods":"brief","calories":0}},{{"meal":"Snacks","foods":"brief","calories":0}}]}},"nutritionTips":["tip1","tip2","tip3"],"progressionScheme":"how and why to progress","deloadRecommendation":"brief"}}"""
...
def generate():
    ...
    try:
        ...
        payload = {
            ...
            'max_tokens': body.get('max_tokens', 12000),
            'messages': messages,
        }
        if system:
